...
#etapa 1: interaccion con el jugador--------------------------------------------
#hecho por Michelle Zugman

def pide_letra():
	"""Le pide al ususario que ingrese una letra"""
	#hecho por Michelle Zugman

	letra = str(input("Ingrese una letra (si desea abandonar la partida ponga FIN): "))
	letra = letra.lower()
	return letra

def crea_base(len_palabra):
	"""Crea una base con tantos signos de pregunta como letras que componen la palabra a adivinar"""
	#hecho por Michelle Zugman

	base_ahorcado = []

	for i in range(0,len_palabra): 
		base_ahorcado.append("?")
	return base_ahorcado

def valida_letra(letra,lista_letras):
	"""Se fija que el ingreso del ususario sea una única letra"""
	#hecho por Michelle Zugman

	if letra.isalpha() and len(letra) == 1 and letra != "º" and letra != "ª" and letra != "ç":
		valida = True
	else:
		valida = False
	return valida
	
def muestra_info(str_base_ahorcado,aciertos,desaciertos,str_desaciertos):
	"""Contiene los datos de cómo va la partida en el momento en que se llama la función"""
	#hecho por Michelle Zugman

	return str_base_ahorcado+"   "+"Aciertos: "+str(aciertos)+" "+"Desaciertos: "+str(desaciertos)+str(str_desaciertos)

def nueva_letra(letra,lista_letras):
	"""Se fija que el ingreso no sea igual a un ingreso anterior"""
	#hecho por Michelle Zugman

	nueva_letra = False
	while nueva_letra == False:
...
def muestra_resultado(str_base_ahorcado,desaciertos,palabra_str):
	"""Le muestra al usuario los resultados de su partida"""
	#hecho por Michelle Zugman

	if str_base_ahorcado.count("?") == 0:
		resultado_partida = "Adivinó la palabra!"

	elif desaciertos > 7:
		resultado_partida = "Superó la cantidad permitida de desaciertos. La palabra era " + palabra_str + "."

	else:
		resultado_partida = "Abandonó la partida."
	return resultado_partida

def main_interaccion_gamer(palabra): 
	"""Va llamando a las demas funciones de la etapa 1 para que se desarrolle la partida"""
	#hecho por Michelle Zugman
	...
	base_ahorcado = crea_base(len_palabra)
	str_base_ahorcado = "".join(base_ahorcado)
	print("Palabra a adivinar: ")
	info = print(muestra_info(str_base_ahorcado,aciertos,desaciertos,str_desaciertos))
	letra = pide_letra()
	lista_letras = [letra]
	
	while desaciertos < 8 and str_base_ahorcado.count("?") > 0 and letra != "fin":

		if valida_letra(letra,lista_letras): 
			for i in range(0,len_palabra): 
				if letra == palabra[i]:
					base_ahorcado[i] = letra
			
			str_base_ahorcado = "".join(base_ahorcado)

			if str_base_ahorcado.count(letra) > 0: 
				aciertos += 1
				print("Muy bien!--> "+muestra_info(str_base_ahorcado,aciertos,desaciertos,str_desaciertos))
				
			else:
				desaciertos += 1
				lista_desaciertos.append("-"+letra)
				str_desaciertos = "".join(lista_desaciertos) 
				print("Lo siento!-->"+muestra_info(str_base_ahorcado,aciertos,desaciertos,str_desaciertos))
				
		else:
			print("Ingreso inválido, inténtelo otra vez.")
		
		if str_base_ahorcado.count("?") > 0 and desaciertos < 8:
			letra = pide_letra()
			if valida_letra(letra,lista_letras):
				letra = nueva_letra(letra,lista_letras)
					
	resultado_partida = print(muestra_resultado(str_base_ahorcado,desaciertos,palabra_str))
...
def filtrar_lista(texto):
    """Ingresa un texto y devuelve una lista de palabras sin caracteres especiales\
    y de longitud mayor igual a LONGITUD_MINIMA"""
    #hecho por Franco Macke

    LONGITUD_MINIMA = 5
    texto_filtrado = saca_tildes(texto)
    texto_filtrado = sacar_caracteres_especiales(texto_filtrado)
    texto_filtrado = convertir_a_minuscula(texto_filtrado)

    return filter(lambda palabra: len(palabra) >= LONGITUD_MINIMA, texto_filtrado)

def saca_tildes(texto):
	"""Recibe un texto y devuelve ese texto sin tildes"""
	#hecho por Michelle Zugman

	con_tildes="áéíóúÁÉÍÓÚ"
	sin_tildes="aeiouAEIOU"
	saca_tildes=str.maketrans(con_tildes,sin_tildes)
	return texto.translate(saca_tildes)

def sacar_caracteres_especiales(texto):
    """Recibe una cadena de texto y devuelve una lista de palabras sin caracteres especiales ni numeros"""
    #hecho por Franco Macke

    lista_palabras = re.split(r'\W+', texto)
    lista_palabras = filter(lambda palabra: palabra.isalpha(), lista_palabras)

    return lista_palabras

def convertir_a_minuscula(lista):
    """Convierte palabras de una lista a minuscula (para ordenar alfabeticamente)"""
    #hecho por Franco Macke
 
    return map(lambda palabra: palabra.lower(), lista)

def convertir_a_diccionario (texto):
    """Ingresa un texto, imprime las palabras que pasaron el filtro y la cantidad de palabras,\
    devuelve un diccionario indicando cuántas veces se repite la palabra en el texto"""
    #hecho por Franco Macke

    lista_filtrada = filtrar_lista(texto)
    diccionario = {}

    for palabra in lista_filtrada:

        if palabra in diccionario:
            cuantas_veces = diccionario[palabra]
            cuantas_veces += 1
            diccionario.update({palabra: cuantas_veces})
...
#etapa 3: eleccion de palabra candidata a adivinar por el jugador-------------------------
#Hecho por Michelle Zugman

import random

def largo_palabra():
	"""Pide al usuario que ingrese el largo que desea para la palabra a adivinar, hasta que sea válido"""
	#hecho por Michelle Zugman

	largo_palabra = input("Ingrese el largo que desea para la palabra, si no desea elegir el largo ponga 0:")

	while largo_palabra.isnumeric() == False or int(largo_palabra) < 0:
		print("Ingreso inválido, pruebe otra vez.")
		largo_palabra = input("Ingrese el largo que desea para la palabra, si no desea elegir el largo ponga 0:")
		
	return largo_palabra

def filtra_diccionario(diccionario,largo):
	"""Filtra el diccionario de las palabras segun el largo elegido por el usuario"""
	#hecho por Michelle Zugman

	dic_filtrado = {}

	if int(largo) == 0:
		dic_filtrado = diccionario

	else:
		
		lista_keys_dic = list(diccionario.keys())
		lista_values_dic = list(diccionario.values())

		for i in range(0,len(diccionario)):
			
			if len(lista_keys_dic[i]) == int(largo):
				dic_filtrado[lista_keys_dic[i]] = lista_values_dic[i]
			
	return dic_filtrado

def aleatorio(diccionario_filtrado):
	"""Devuelve una palabra del diccionario filtrado, elegida de forma aleatoria"""
	#hecho por Michelle Zugman

	len_diccionario_filtrado = len(diccionario_filtrado)
	pos_aleatoria = random.randrange(0,int(len_diccionario_filtrado))
	lista_keys_dic = list(diccionario_filtrado.keys())
	palabra_aleatoria = lista_keys_dic[int(pos_aleatoria)]
	return palabra_aleatoria

def elige_palabra(diccionario):
	"""Va llamando a las demas funciones de esta etapa y devuelve la palabra con la que jugará el usuario"""
	#hecho por Michelle Zugman

	largo = largo_palabra()
	diccionario_filtrado = filtra_diccionario(diccionario,largo)

	while len(diccionario_filtrado) == 0:
		print("No puede tener ese largo, elija otro.")
		largo = largo_palabra()
		diccionario_filtrado = filtra_diccionario(diccionario,largo)

	palabra_aleatoria = aleatorio(diccionario_filtrado)
	return palabra_aleatoria
...
def cuenta_puntos(puntos,partida):
	"""Cuenta los puntos de las partidas"""
	#hecho por Michelle Zugman

	puntos += 10*int(partida[0])
	puntos -= 5*int(partida[1])
	print("Tiene un total de ",puntos,"puntos.")
	return puntos

def seguir_otra_partida():
	"""Pregunta al usuario si quiere jugar una nueva partida"""
	#hecho por Michelle Zugman

	seguir = str(input("Desea jugar otra partida?(si,no): "))
	seguir = seguir.lower()
	return seguir

def main():
	"""Llama a la función principal de las etapas 1, 2 y 3, \
	a la funcion cuenta puntos y la funcion seguir otra partida y almacena los datos obtenidos"""
	#hecho por Michelle Zugman

	puntos = 0
	texto = obtener_texto()
	diccionario = convertir_a_diccionario(texto)
	print("Vamos a jugar al ahorcado!")
	lista_palabras = []
	seguir = "si"

	while seguir == "si":
		palabra = elige_palabra(diccionario)
		while palabra in lista_palabras:
			palabra = elige_palabra(diccionario)

		lista_palabras.append(palabra)
		partida = (main_interaccion_gamer(palabra))
		puntos = cuenta_puntos(puntos,partida) 
		seguir = seguir_otra_partida()

		while seguir != "si" and seguir != "no":
			print("Ingreso inválido, intentelo de nuevo.")
			seguir = seguir_otra_partida()
